...
function LinRadiometria(){
	...
	GrafLineal("#GrCanvas", FinalXs, LYs, FinalYs, "Log(Posición)", "Log(Voltaje)");
};
...
function GenerarTabel(relleno, val1, val2, val3){
	let TablaCuerpo = document.getElementById(relleno);
	TablaCuerpo.innerHTML += `<tr><td>${Math.round10(val1, -3)}</td>  <td>${Math.round10(val2, -3)}</td>  <td>${Math.round10(val3, -5)}</td></tr>`;
}
...
// function Funciones{
	// identificador = document.getElementById()
	// if 
// }
